...
// Practice group per player, for SFC vs QofE tracking. Everyone is SFC except
// the QofE folks listed here. (Static for now — move to a players column +
// picker if this needs to change often.)
export const PRACTICE = { pablo: 'QofE', sarah: 'QofE', avery: 'QofE', 'rowan-williams': 'FALS' }
export const practiceOf = (playerId) => PRACTICE[playerId] || 'SFC'
...
export async function getStandings() {
  ...
  return ordered.map((p, i) => {
    ...
    return {
      ...
      practice: practiceOf(p.id),
    }
  })
}

// Full career detail for one player: rank, record, rating series over time,
// head-to-head vs every opponent, and their match log (newest first).
export async function getPlayerDetail(id) {
  const d = await load()
  const player = d.players.find((p) => p.id === id)
  if (!player) return null

  const stats = new Map(d.players.map((p) => [p.id, { rating: BASE, wins: 0, losses: 0, streak: 0 }]))
  const series = [{ at: null, rating: BASE }]
  const history = []
  const h2h = new Map()
  for (const m of [...d.matches].reverse()) { // oldest → newest
    const w = stats.get(m.winner_id), l = stats.get(m.loser_id)
    if (!w || !l) continue
    const expected = 1 / (1 + 10 ** ((l.rating - w.rating) / 400))
    const delta = Math.max(1, Math.round(K * (1 - expected)))
    ...
    w.streak = w.streak > 0 ? w.streak + 1 : 1
    l.streak = l.streak < 0 ? l.streak - 1 : -1
    if (m.winner_id === id || m.loser_id === id) {
      const won = m.winner_id === id
      const oppId = won ? m.loser_id : m.winner_id
      const oppName = won ? m.loser_name : m.winner_name
      const me = stats.get(id)
      history.push({ at: m.played_at, won, oppId, oppName, delta: won ? delta : -delta, ratingAfter: me.rating, by: m.recorded_by })
      series.push({ at: m.played_at, rating: me.rating })
      const rec = h2h.get(oppId) || { oppId, oppName, w: 0, l: 0 }
      won ? (rec.w += 1) : (rec.l += 1)
      h2h.set(oppId, rec)
    }
  }
  const ordered = [...d.players].sort((a, b) => (stats.get(b.id).rating - stats.get(a.id).rating) || (a.seed - b.seed))
  const rank = ordered.findIndex((p) => p.id === id) + 1
  const s = stats.get(id)
  const games = s.wins + s.losses
  return {
    id: player.id, name: player.name, can_submit: player.can_submit,
    rank, total: d.players.length,
    rating: s.rating, wins: s.wins, losses: s.losses, streak: s.streak, games,
    winPct: games ? Math.round((s.wins / games) * 100) : null,
    practice: practiceOf(player.id),
    series,
    history: history.reverse(),
    h2h: [...h2h.values()].sort((a, b) => (b.w + b.l) - (a.w + a.l) || b.w - a.w),
  }
}
